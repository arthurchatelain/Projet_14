...
export async function isValid (firstName = '', lastName = '', street = '', city = '', state = '', zipCode = '', department = '') {
        if (firstName.length <= 2 || firstName[0] === ' ' || validChaine(firstName) === false) throw new Error('invalid firstName')
        else if (lastName.length <= 2 || lastName[0] === ' ' || validChaine(lastName) === false) throw new Error('invalid lastName')
        else if (street.length <= 3 || street[0] === ' ' || validChaine(street) === false) throw new Error('invalid street')
        else if (city.length <= 3 || city[0] === ' ' || validChaine(city) === false) throw new Error('invalid city')
